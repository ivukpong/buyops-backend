...
let AssetsService = class AssetsService {
    ...
    async update(id, data) {
        ...
        if (data.basePrice !== undefined)
            updateData.basePrice = parseFloat(data.basePrice);
        if (data.markup !== undefined)
            updateData.markup = parseFloat(data.markup);
        if (data.finalPrice !== undefined)
            updateData.finalPrice = parseFloat(data.finalPrice);
...
let TransactionsService = class TransactionsService {
    ...
    async getUnpaidCommissions({ month }) {
        return this.findAll({
            status: undefined,
            month,
        }).then(transactions => transactions.filter(t => t.commissionPaymentStatus === 'UNPAID' || t.commissionPaymentStatus === 'SENT'));
    }
    async getPaidCommissions({ month }) {
        return this.findAll({
            status: undefined,
            month,
        }).then(transactions => transactions.filter(t => t.commissionPaymentStatus === 'PAID'));
    }
